...
for each_bucket in all_buckets:
    ...
    if (policy['Effect'] == "Allow"
        ...
        and policy['Action'] == "s3:GetObject"):
            print(f"\nResource {policy['Resource']} is vulnerable")
            print(f"Because {policy['Action']}")
            print(f"is {policy['Effect']}ed for Principal: {policy['Principal']}")
